...
public:
    ...
    bool isObject(int p)
    {
        ...
        if (getCursorChar() == -37 || getCursorChar() == -78)
            return true;
    ...
    bool collideV(int p)
    {
        if (((1 <= location[0] + p && (XX - 1) >= location[0] + p) && (2 == location[1] + p || (YY) == location[1] + p)) || (isObject(p)))
            return true;
        return false;
    }
    ...
    bool collideH(int p)
    {
        if (((3 <= location[1] + p && (YY) >= location[1] + p) && (1 == location[0] + p || (XX - 1) == location[0] + p)) || (isObject(p)))
            return true;
        return false;
    }
...
C:
    while (true)
    {
        ...
        A:
            ...
            if (flag)
                temp = menu("NextLEVEL GOBACKtoMENU ");
            else if (!flag)
                temp = menu("RE-PLay GOBACKtoMENU ");
            if (temp != '2')
                goto A;
